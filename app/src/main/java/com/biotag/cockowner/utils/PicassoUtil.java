...
public class PicassoUtil {
    ...
    public void loadImage(Context context, String url, ImageView imageView) {
        if (!TextUtils.isEmpty(url)) {
            Picasso.with(context).load(url).placeholder(R.mipmap.chickenlife).error(R.mipmap.chickenlife).into(imageView);
        }
    }
    /**
     * 加载普通的图片
    ...
    public void loadnormalImage(Context context,String url,ImageView imageView){
        if(!TextUtils.isEmpty(url)){
            Picasso.with(context).load(url).placeholder(R.mipmap.chickenlife).error(R.mipmap.chickenlife).config(Bitmap.Config.RGB_565).into(imageView);
        }
    }
}
